...
date_list = ["_".join([file_name[27: 31], file_name[31: 33], file_name[33: 35]]) for file_name in image_filename_list]

# create preci datasets for average values
temp_data = adm_map.copy()
# make date columns to fill in a loop in the next step
for date in date_list:
    temp_data[date] = np.nan
...
for date, image_filename in zip(date_list, image_filename_list):
    ...
    temp_data.loc[:, date] = mean_value_ls
...
temp_data.drop("geometry", axis=1).to_csv(PROCESSED_DATA_DIR / "climate/temp_regional_matrix.csv", index=False)

with open(PROCESSED_DATA_DIR / "climate/temperature_histograms.pickle", 'wb') as f:
