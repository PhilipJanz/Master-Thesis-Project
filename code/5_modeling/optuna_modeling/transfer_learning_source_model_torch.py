...
from optuna_modeling.optuna_torch_optimizer import OptunaOptimizer
import optuna
...
torch.backends.cudnn.benchmark = False     # if reproduc
np.random.seed(seed)
...
run_name = f"0820_{objective}_{data_split}_transferable-nn_torch_{length}_{timeout}_{n_trials}_{n_startup_trials}_{num_folds}"
...
if run_name in list_of_runs():
    ...
    run = Run(name=run_name,
              ...
              python_file="transfer_learning_source_model_torch")
...
for source_name, source_yield_df in yield_df.groupby(data_split):
    ...
    for year_out in np.unique(years_source):
        ...
        opti = OptunaOptimizer(X=X_train, y=y_train, years=years_train, predictor_names=feature_names_,
                               sampler=sampler, num_folds=num_folds, seed=seed)
        ...
        # train best model
        _, _, trained_model = opti.train_best_model()
